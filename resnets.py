...
def conv3x3(in_planes, out_planes, stride=1, groups=1, dilation=1):
    ...
    std = math.sqrt(1. / (in_planes * 9))
    torch.nn.init.trunc_normal_(conv.weight, std=std, a=-2 * std, b=2 * std)
    return conv
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.bn3 = norm_layer(planes * self.expansion, init_zero=True)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
...
def batch_norm2d_wrapper(n_channels, *args, init_zero=False, **kwargs):
    bn = nn.BatchNorm2d(n_channels, *args, **kwargs)
    if init_zero:
...
class ResNet(nn.Module):
    def __init__(
        ...
        if norm_layer is None:
            # norm_layer = nn.BatchNorm2d
            norm_layer = batch_norm2d_wrapper
